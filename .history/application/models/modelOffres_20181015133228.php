...
class modelOffres extends CI_model{
    ...
    public function getInfosOffre($idOffre){
        $sql = $this->db->query("select photoService, idOffre, descriptionOffre, dateOffre, nomService from offre inner join service on offre.idService=service.idService where idOffre =".$idOffre);
        return $sql->result();
    }
}
