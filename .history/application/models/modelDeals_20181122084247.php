...
class modelDeals extends CI_model{
    ...
    function verifSonOffreMaDemande($sonOffreCliquee, $maDemandeCliquee){
        $sql = $this->db->query("select idService as test from offre WHERE idOffre =".$sonOffreCliquee);
        $sql2 = $this->db->query("select idService as test from demande WHERE idDemande =".$maDemandeCliquee);      
        echo var_dump($sql->result()[0]->test);
