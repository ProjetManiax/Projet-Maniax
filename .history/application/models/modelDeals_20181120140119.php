...
class modelDeals extends CI_model{
    ...
    function verifMonOffreSaDemande($monOffreCliquee, $saDemandeCliquee){
        $sql = $this->db->query("select service.idService as test from service inner JOIN demande on service.idService=demande.idDemande WHERE demande.idDemande =".$saDemandeCliquee);
